...
public class NewmannGirvan
{
     ...
     */
    private Set<Node> putToCollectionIt(Graph G, Node[] nodes, boolean[] vist, int i) {
     ...
     */
    private ArrayList<Set<Node>> putToCollection(Graph G, Node[] nodes) {
        int nodeCount = G.getOrder();
        ArrayList<Set<Node>> cc = new ArrayList<Set<Node>>();
        ...
        for (int i = 0; i < nodeCount; ++i) {
            if (!vist[i]) {
                cc.add(putToCollectionIt(G, nodes, vist, i));
            }
        }
        return cc;
    }
     ...
     */
    public ArrayList<Set<Node>> runAlgorithm(Graph G, int nCom) {
